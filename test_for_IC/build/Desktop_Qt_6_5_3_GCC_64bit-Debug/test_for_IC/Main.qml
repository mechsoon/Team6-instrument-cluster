...
Item {
    ...
    Image {
        ...
        transform: Rotation {
            id: needleRotation
            origin.x: 115
            origin.y: 5 // 바늘의 중심점을 끝으로 설정 (바늘 끝이 회전하도록)
            angle: -28  // 초기 각도 (속도가 0일 때)
        }
    }
    ...
    NumberAnimation on needleRotation.angle {
        id: rotationAnim
        from: -28
        to: 208  // 바늘이 회전할 각도 범위 (속도에 따라 조정)
        duration: 500
    }
    ...
    function updateSpeed(newSpeed) {
        ...
        var maxSpeed = 180  // 계기판에서 표시할 최대 속도
        var minAngle = -28  // 바늘의 최소 각도
        var maxAngle = 208   // 바늘의 최대 각도
